...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['email_medico'])) {
    $email_medico = $_POST['email_medico'];

    // Preparar la llamada a la función Eliminar_Medico
    $query = "CALL Eliminar_Medico(?)";
    $stmt = $conexion->prepare($query);

    if ($stmt) {
        // Bind del parámetro email_medico
        $stmt->bind_param("s", $email_medico);
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->fetch()) {
                $mensaje = $resultado;
                $tipo_mensaje = ($mensaje === 'Médico eliminado correctamente') ? 'exito' : 'error';
            } else {
...
<div id="contenedor">
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" id="formulario">
        <div class="form-group">
            <label for="email_medico">Email del médico a eliminar:</label>
