...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['cita_id'], $_POST['descripcion'], $_POST['recomendacion'])) {
    ...
    try {
        ...
        // Paso 1: Insertar el diagnóstico usando el procedimiento almacenado
        ...
        // Liberar resultados del procedimiento almacenado
        ...
        if (!$result) {
            throw new Exception("Error al obtener ID del diagnóstico: " . $conexion->error);
        }
        ...
        // Paso 2: Procesar medicamentos si existen
        if (!empty($_POST['nombre_medicamento']) && !empty($_POST['frecuencia'])) {
            ...
            if (!$stmtMedicamento) {
                throw new Exception("Error al preparar consulta de medicamentos: " . $conexion->error);
            }
        ...
        $conexion->commit();
        echo "✅ Diagnóstico y medicamentos insertados correctamente.<br>";
